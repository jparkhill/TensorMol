...
void CartToSphere(double* InOut)
{
	double x = InOut[0];
	double y = InOut[1];
	double z = InOut[2];
...
void SphereToCart(double* InOut)
{
	double r = InOut[0];
...
//  Exp[-(x-r)^2/(2 sigma^2)]*Y_{LM}(theta,phi)
// which will occupy a vector of length Nrad*(1+lmax)**2
void RadInvProjection(SHParams* Prm, double x, double y, double z, double* output, double fac=1.0)
{
	double rnotinv=sqrt(x*x+y*y+z*z);
	double r = 1.0/rnotinv;
	// Populate tables...
