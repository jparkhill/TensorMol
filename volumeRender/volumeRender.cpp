...
const char *volumeFilename = "Vol.raw";
cudaExtent volumeSize = make_cudaExtent(32, 32, 32); // Will be overwritten after being read. 
typedef unsigned char VolumeType;
...
// Load sized data from disk, and update the global sizes. 
void *loadRawFileSized(char *filename)
{
    

        fprintf(stdout, "Trying to Read: '%s'\n", filename);
	FILE *fp = fopen(filename, "rb");
	uint8_t dims_uint8[3] = {0,0,0};
    if (!fp)
    {
    ...
    // Read the dimensions 
	size_t read = fread(dims_uint8, 1, 3, fp);
        fprintf(stdout, "Read dimensions: '%d, %d, %d'\n", dims_uint8[0],dims_uint8[1],dims_uint8[2]);


	int dims[3];
	dims[0] = (int)(dims_uint8[0]);
	dims[1] = (int)(dims_uint8[1]);
	dims[2] = (int)(dims_uint8[2]);
	
	fprintf(stdout, "Read dimensions: '%d, %d, %d'\n", dims[0],dims[1],dims[2]);

	volumeSize = make_cudaExtent(dims[0],dims[1],dims[2]);	
...
main(int argc, char **argv)
{
	...
	void *h_volume = loadRawFileSized(path);
